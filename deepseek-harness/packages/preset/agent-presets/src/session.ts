 ...
 * deep-frozen because that is a creation fact. A later switch — blank or
 * after turns have run — is an `agent-preset/selected` event: later turns
 * run under the newly mounted composition, and historical tool calls stay
 * in the log. Recording the change is what keeps the log honest, and it is
