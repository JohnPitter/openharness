...
- `ctx.agentPresets.recompose(agentCtx, id): Promise<AgentPreset>` 把一个 agent 重链到另一个 preset 的常驻组装。在 agent 空闲时合法——**由调用方负责该检查**；新挂载在链移动之前确保完成，失败时 agent 原封不动。与 `mount()` 一样拒绝损坏的 preset。
...
- `ctx.agentPresets.compositionInventory(): Promise<AgentPresetComposition[]>` 每个 roster preset 一组：已有常驻挂载的从 Loader 行回答，尚未挂载的从组合文件回答且不挂载，损坏文件带着原因留在名单上。读取从不激活 preset。
...
头部保持冻结，因为它是创建期事实。切换以 `agent-preset/selected` 会话事件记录，在替换提交之后追加；这正是 model-visible ⟺ logged 规则的要求：preset 决定模型看到的工具 schema 与提示词段落，因此必须能从日志重建。服务会把这项已提交事实重新发为不带 scope 的 cordis 事件 `agent-preset/selected(sessionId, agentPreset)`，其声明位于 client-safe 的 `./types` 出口，使远端消费方无需导入 Host 运行时类型即可让会话派生状态失效。只读头部会让切换过的会话按创建时的组装重建，之后的轮次就会在错误的工具集下重放。

### 切换 agent

`recompose()` 先卸载已装入的子树、再装入新的，因为两份组装无法共存——它们会把相同的工具名注册进同一个层。挂载失败会恢复先前的组装，而不是让 agent 一无所有；未知 id 则在任何东西被拆除之前就被拒绝。

网关仅在一轮仍在进行时拒绝（[`dsh-apiproxy`](../../host/apiproxy/README.zh.md) 返回 `agent-preset-locked`）。历史上的工具调用留在日志里；之后的轮次使用新组装。
...
直接挂载的子树不会出现在 `ctx.loader.entries()` 中，因此没有任何启动审计能覆盖它。`mount()` 因此自行校验结果可用，并拒绝三种情况。当 roster 本身是 Loader 条目时，常驻 `PresetTree` 收回该条目的 `subtree` 槽，使 preset 行仍不进入宿主的 `loader.entries()` 遍历。
