...
- `AskUserQuestionRequest`：`{ questions: [{ id, question, detail?, header?, options?, multiSelect?, intent? }], agent?, signal? }`；`detail` 提供辅助文本，提供方会将其随问题一起渲染，而不会将其变成选项标签。如提供 `agent`，它必须与注册表中的存活实例是同一对象。
...
请求包含 agent 时，`ask()` 会通过当前 `AgentRegistry` 验证该 agent 与注册表中的存活实例是同一对象。人类 UI 等待只发生在用户正在回答的运行时根上。被委托的调用方——归属于另一个 agent 的存活子级，或父会话仍存活的 continuable `origin: 'subagent'` agent——不会等待：`ask()` 选择带 `(Recommended)` 的选项（否则选第一项）并返回。没有选项的批次仍以 `DELEGATED_CALLER` 失败。持久谱系单独不构成权限依据：带有历史子级谱系的会话在恢复为没有存活父级的新运行时根后可以提问。不含 agent 的程序化请求继续沿用现有提供方路径。
...
间接地，通过 `dsh-tool-ask-user`：它会将成功的提供方回答或自动选定的委托选项保留为紧凑 JSON，或返回以下失败之一：`Error: ask_user_question was aborted before the user answered`、`Error: ask_user_question requires at least one question`、`Error: human interaction requires the exact live calling agent when an agent is supplied`、`Error: human interaction is unavailable while the calling agent is owned by another live agent; include the unresolved question or decision in the child agent's final result`、`Error: no user-questions provider is registered` 或 `Error: <message>`。等待人类回答不会增加 token。自动选定的委托回答也不会。
