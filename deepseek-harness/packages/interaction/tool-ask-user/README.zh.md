...
- **待处理问题会阻塞工具调用，直至用户作答**：该工具未声明 `timeout-policy` 预算；取消仅沿用当前轮次的 `exec.signal`。被委托的调用方不会进入该等待：它们收到推荐选项（否则是第一项），或在批次没有选项时得到 `DELEGATED_CALLER`。
- **被委托的调用方不等待人类通道**：归属于另一个 agent 的存活子级，或父会话仍存活的 continuable `origin: 'subagent'` agent，在 `ctx.userQuestions.ask()` 处自动选定选项。没有选项的问题仍以 `DELEGATED_CALLER` 失败；子级必须在最终结果中包含尚未解决的自由文本问题。带有谱系的会话在恢复为没有存活父级的运行时根后可以正常提问。
