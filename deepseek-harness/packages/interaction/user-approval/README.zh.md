...
`ApprovalPolicy` 为 `'ask'`、`'never'` 或 `'always'`。实际值取最后一条 `approval/policy` 事件，并回退到配置；`setApprovalPolicy()` 是写入路径。`'never'` 会在交互式分发之前拒绝请求。`'always'` 会在交互式分发之前授予请求。三种策略都会将各自完整的当前含义贡献给缓存安全的运行时上下文快照。
...
首次请求和有效策略每次变化时，都会在保留的历史后追加一份完整运行时上下文快照。在 `ask` 下，审批上下文内容会说明系统可以咨询已配置的应答者，缺少可用应答者时则以拒绝方式关闭。在 `never` 下，它会说明确定性的拒绝与非升权后果。在 `always` 下，它会说明确定性的授予，包括沙箱升权。未变化的请求会保留先前快照，不增加另一条消息。
...
- **仅存在一次性授权**：结果词汇包含 `allowed-once`，但不含已记住的规则、撤销或授权存储；会话策略为 `ask`／`never`／`always`。
