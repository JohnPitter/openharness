...
agent preset 的各个表层：General 设置中的一行，用于选择新建会话据以组装的 [preset](../../preset/agent-presets/README.zh.md)；新建会话界面上的一枚 chip，用于选择**下一个会话**的 preset；会话标题旁的一个控件，用于切换**本会话**的组装；以及一个设置页分区，用于管理名单——复制、删除、默认值，以及通往 preset 自身文件的入口。

## 为什么 General 行是「新建会话」的偏好设置

更改默认值只对此后开启的会话生效。活会话保持它当前运行的组装，直到标题旁的控件切换它。

## 新建会话 chip

第二个表层，位于新建会话界面上、工作区选择器旁边。它落在这里而非 composer，是因为该界面先于它要应用到的会话存在。
...
已经开始的会话不是这枚 chip 的职责：暂存值被丢弃而非排队。那个会话由标题旁的控件切换。

## 会话标题旁的控件

第三个表层，位于会话标题旁：**本会话**所运行的 preset，以菜单呈现。点选即重组活着的 agent。宿主仅在一轮仍在进行时拒绝（`agent-preset-locked`）。子会话（`origin: subagent`）只显示名称——它加入的是父会话的挂载。它从会话自身的摘要读取 preset，并在 General 行所读的同一份名单上解析显示名称。转发的 owner 事件 `agent-preset/selected` 会在每个标签页中把已经提交的切换折进这份共享摘要；发起方标签页可能已经采用 RPC 回执，而合并是幂等的。
...
未组装任何 preset 的部署返回空名单，本行、chip、标题控件与分区都不渲染任何内容——此时每个会话共用宿主组装，也就无从选择或管理。未配置可写根目录的部署返回 `authorable: false`，分区随之退化为只读浏览：随附组装仍可在查看器中打开，但每个复制操作都被禁用并以原因作提示，而不是给出一个创建必然失败的对话框。
...
没有直接的失效影响。更改默认值绝不触及活会话的前缀。从标题旁的控件切换活会话会重组该 agent，因此之后的轮次开始新的前缀。
