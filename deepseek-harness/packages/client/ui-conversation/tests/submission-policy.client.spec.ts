...
describe('ComposerSubmissionPolicy', () => {
  it('defaults to Queue and only applies the preference while running', () => {
    ...
    expect(resolveSubmitMode(DEFAULT_BUSY_ENTER_BEHAVIOR, false, 'enter', true)).toBe('queue')
    expect(resolveSubmitMode(DEFAULT_BUSY_ENTER_BEHAVIOR, false, 'accelerated', true)).toBe('queue')
    expect(resolveSubmitMode(DEFAULT_BUSY_ENTER_BEHAVIOR, true, 'enter', true)).toBe('queue')
    expect(resolveSubmitMode(DEFAULT_BUSY_ENTER_BEHAVIOR, true, 'accelerated', true)).toBe('steer')
    expect(resolveSubmitMode(DEFAULT_BUSY_ENTER_BEHAVIOR, true, 'enter', false)).toBe('queue')
    expect(resolveSubmitMode(DEFAULT_BUSY_ENTER_BEHAVIOR, true, 'accelerated', false)).toBe('queue')
  ...
  it('adopts a Host preference without writing it back and leaves an identical write untouched', () => {
    ...
    host.publish({ status: 'ready', value: { busyEnter: 'steer', taskCompleteSound: 'notify-email' }, revision: 1, writable: true })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
    policy.setBusyEnter('steer')
    expect(host.set).not.toHaveBeenCalled()
    host.publish({ value: { busyEnter: 'steer', taskCompleteSound: 'notify-email' }, revision: 2 })
    expect(policy.busyEnter.getSnapshot()).toBe('steer')
  })
