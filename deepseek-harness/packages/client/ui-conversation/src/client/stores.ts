...
type ChatActions = {
  ...
  setActivityExpanded: (draft: ChatStoreState, key: string, expanded: boolean) => void
}
...
export function createChatStore(): EngineStoreHandle<ChatStoreState, ChatActions> {
  return defineStore({
    ...
    init: (): ChatStoreState => ({ selection: null, draft: '', view: null, inspect: null, activityExpanded: {} }),
    persist: 'dsh.conversation.chat',
    transient: ['activityExpanded'],
    actions: {
      ...
      setActivityExpanded: (d, key: string, expanded: boolean) => {
        d.activityExpanded = { ...d.activityExpanded, [key]: expanded }
      },
    },
  })
}
