...
function mount({
  ...
  const t = ((key: SettingsKey) => en[key]) as SettingsRootComponentProps['t']
  ...
  let boundOpen: ((id: string) => void) | undefined
  const props: SettingsRootComponentProps = {
    ...
    renderSlot,
    bindNavigation: (openSection) => {
      boundOpen = openSection
      return () => { if (boundOpen === openSection) boundOpen = undefined }
    },
  }
  ...
  return { view, renderSlot, bump, listeners, reconnect, setConnectionState, openBound: (id: string) => { boundOpen?.(id) } }
}
...
describe('SettingsPanel close paths', () => {
  it('closes via the header button and restores trigger focus', () => {
    mount()
...
describe('SettingsPanel navigation', () => {
  ...
  it('opens a section when the bound settingsNav handler fires', () => {
    const { openBound } = mount({ onboardingActive: false })
    act(() => { openBound('models') })
    expect(screen.getByRole('dialog')).toBeTruthy()
    expect(screen.getByTestId('section-models')).toBeTruthy()
  })
