...
当宿主报告没有适配器服务该会话的路由（`session.models.routable`）时，本插件经 `ctx.conversation.blocks` 注册一个 composer 阻塞块，输入框随之停用并显示本插件自己的文案；恢复后无需重新加载即自动清除。它首先跟随 `routable`：`null`（首次加载之前，或加载失败之后）绝不阻断，否则一个慢的宿主就会锁死一个本来可用的 composer；目录成员关系同样不阻断，因为一条仍在服务、只是不再公布该模型的路由不在分组里，却完全可用。Workflow 会话在宿主报告的 `currentMetering` 为 `requests` 且工人 chip 为空时，会再升起第二道阻塞，直到选出工人；两者同时成立时仍以不可路由为准。触发器自己的 `Select model` 回退仍然覆盖缺目录行那种情形——那是显示，不是闸门。
...
Settings → Limits（`id: quotas`，order 12）列出 `llm.accountUsage` 的 coding-plan 配额卡片。Settings → Status（`id: usages`，order 13）是来自 `usage.panel` 的 Host 本地按日请求与 token 历史，以及按用量排序的模型。两页各自只加载自己的 API。侧栏用量 chip 的「查看全部配额」打开 Limits。
