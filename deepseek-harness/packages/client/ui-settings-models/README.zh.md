...
**获取可用模型**会针对表单**当前显示**的端点调用 `llm.discoverModels`，包括已修改但尚未保存的 API 地址和已键入但尚未存储的密钥，因此新增一个提供方是一趟走完，而不是「先保存再回来」。回复会打开一个可搜索的选择框而不是直接写入：已配置过的候选默认不勾选，因此采纳一次选择绝不会覆盖用户已更正的容量。搜索匹配模型 id 与可选显示名称，不会清除被隐藏的已选项。**全选**作用于当前可见结果；**取消全选**清空整个选择，以免误采纳被筛掉的项。点击**添加所选**之前不会写入任何内容。无法被询问的提供方只是绕路而非死路——适配器自己的消息会显示在各行旁边，而这些行仍可手工编辑。
...
- **询问覆盖 OpenAI 兼容端点与 Anthropic Messages**：OpenAI 协议接受标准 `data` 数组或充实的 `models` 映射，Anthropic 走其原生模型列表路由；其余协议会报告自己无法被询问，其模型需手工填写。
