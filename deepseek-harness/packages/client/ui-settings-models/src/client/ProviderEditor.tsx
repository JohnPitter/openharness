...
import { fetchOauthStatus, logoutOauth, startOauthLogin } from './oauth-client.ts'
import type { OauthProviderStatus } from './oauth-client.ts'
...
export function ProviderEditor(props: ProviderEditorProps): ReactNode {
  ...
  const [oauth, setOauth] = useState<OauthProviderStatus | undefined>(undefined)
  ...
  useEffect(() => {
    ...
    const load = (): void => {
      void fetchOauthStatus().then((snapshot) => {
        if (stale) return
        setOauth(snapshot?.providers.find(entry => entry.id === props.provider))
      })
    }
    load()
    const waiting = oauth?.loginStatus === 'waiting'
    const timer = waiting ? window.setInterval(load, 2000) : undefined
    return () => {
      stale = true
      if (timer !== undefined) window.clearInterval(timer)
    }
  }, [layout, props.provider, oauth?.loginStatus])
  ...
  const curatedFields = (family: 'deepseek' | 'kimi' | 'pi-ai'): ReactNode => {
    ...
    const runOauth = async (method?: string): Promise<void> => {
      ...
      setFailure(undefined)
      try {
        const snapshot = await startOauthLogin(props.provider, method)
        const next = snapshot.providers.find(entry => entry.id === props.provider)
        setOauth(next)
        if (next?.loggedIn === true && props.credentialOnly === true) props.onClose(true)
      } catch (error) {
        setFailure(messageOf(error))
      } finally {
        setBusy(false)
    ...
    const runLogout = async (): Promise<void> => {
      ...
      setFailure(undefined)
      try {
        const snapshot = await logoutOauth(props.provider)
        setOauth(snapshot.providers.find(entry => entry.id === props.provider))
      } catch (error) {
        setFailure(messageOf(error))
      } finally {
    ...
    const oauthBusy = disabled || oauth?.loginStatus === 'waiting'
    return (
      <>
        {oauth === undefined ? null : (
          <div className={styles['oauth']}>
            <span className={styles['fieldLabel']}>{t('oauthTitle')}</span>
            <p className={styles['oauthStatus']}>
              {oauth.loggedIn
                ? t('oauthSignedIn').replace('{source}', oauth.authSource ?? oauth.authType ?? 'OAuth')
                : oauth.loginStatus === 'waiting'
                  ? t('oauthWaiting')
                  : oauth.loginStatus === 'error'
                    ? (oauth.loginDetail ?? t('oauthSignedOut'))
                    : t('oauthSignedOut')}
            </p>
            {oauth.userCode === undefined ? null : (
              <p className={styles['oauthCode']}>
                {t('oauthUserCode')}
                {' '}
                <code>{oauth.userCode}</code>
              </p>
            )}
            {oauth.openUrl === undefined ? null : (
              <a className={styles['oauthLink']} href={oauth.openUrl} target="_blank" rel="noreferrer">
                {t('oauthOpenUrl')}
              </a>
            )}
            <p className={styles['oauthHint']}>{t('oauthHint')}</p>
            <div className={styles['oauthActions']}>
              {oauth.loggedIn
                ? (
                  <button type="button" className={styles['secondaryButton']} disabled={oauthBusy} onClick={() => { void runLogout() }}>
                    {t('oauthSignOut')}
                  </button>
                )
                : oauth.methods.includes('device_code')
                  ? (
                    <>
                      <button type="button" className={styles['primaryButton']} disabled={oauthBusy} onClick={() => { void runOauth('browser') }}>
                        {t('oauthSignInBrowser')}
                      </button>
                      <button type="button" className={styles['secondaryButton']} disabled={oauthBusy} onClick={() => { void runOauth('device_code') }}>
                        {t('oauthSignInDevice')}
                      </button>
                    </>
                  )
                  : (
                    <button type="button" className={styles['primaryButton']} disabled={oauthBusy} onClick={() => { void runOauth() }}>
                      {t('oauthSignIn')}
                    </button>
                  )}
            </div>
    ...
    <div className={props.credentialOnly === true ? styles['addBlock'] : styles['editor']}>
      ...
      <EditorFooter
        ...
        submitDisabled={disabled || layout === 'unknown'
          ...
          || (props.credentialRequired === true && keyValue.length === 0 && oauth?.loggedIn !== true)}
