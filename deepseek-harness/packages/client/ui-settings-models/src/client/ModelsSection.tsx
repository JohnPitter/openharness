...
import { fetchOauthStatus } from './oauth-client.ts'
...
function Loaded({ injected }: { injected: ModelsSectionFace }): ReactNode {
  ...
  const [oauthLoggedIn, setOauthLoggedIn] = useState<ReadonlySet<string>>(() => new Set())

  useEffect(() => {
    let stale = false
    void fetchOauthStatus().then((snapshot) => {
      if (stale || snapshot === undefined) return
      setOauthLoggedIn(new Set(snapshot.providers.filter(entry => entry.loggedIn).map(entry => entry.id)))
    })
    return () => { stale = true }
  }, [state.rows, editing?.provider])
  ...
  const anyUsable = state.rows.some(row => providerUsable(row) || oauthLoggedIn.has(row.entry.provider))
      ...
      <ul className={styles['rows']}>
        {configured.map((row) => {
          ...
          const credentialConfigured = row.credential?.configured === true
            || oauthLoggedIn.has(row.entry.provider)
          const credentialMissing = !credentialConfigured
            && row.apiKeyEnv !== undefined
            && row.credential?.configured === false
          const configuredLabel = row.credential?.configured === true
