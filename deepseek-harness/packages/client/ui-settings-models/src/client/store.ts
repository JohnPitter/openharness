...
/**
 * Active key-based providers the first-run prompt can collect a credential for.
 * @param state - current shared Models join snapshot.
 * @returns rows the onboarding selector may offer.
 */
export function onboardableRows(state: ModelsSettingsState): ProviderRow[] {
 ...
 * to, so ANY usable provider ends it. When none exists, any writable key-based
 * route (DeepSeek, Kimi, …) can collect a credential. A directory with no
 * addressed key-based adapters is not repairable by this prompt.
 * @param state - current shared Models join snapshot.
...
export function onboardingReadiness(state: ModelsSettingsState): OnboardingReadiness {
  ...
  const addressed = state.rows.filter(row =>
    row.entry.settingsNs.length > 0 && row.apiKeyEnv !== undefined)
  if (addressed.length === 0) return { kind: 'adapter-absent' }
  if (state.credentialError !== null || addressed.every(row => row.credential === undefined)) {
    return {
      kind: 'unavailable',
      reason: 'credentials-unavailable',
    }
  }
  const promptable = addressed.filter(row =>
    row.entry.active && row.credential !== undefined && row.credential.writable)
  ...
  if (addressed.every(row => !row.entry.active)) {
    return {
      kind: 'unavailable',
      reason: 'provider-inactive',
    }
  }
  if (addressed.some(row => row.credential === undefined)) {
    return {
      kind: 'unavailable',
      reason: 'credentials-unavailable',
    }
