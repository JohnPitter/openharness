...
function kimiNamespace(apiKeyEnv: string | null): SettingsNamespaceView {
  const value = apiKeyEnv === null ? {} : { apiKeyEnv }
  return {
    ns: 'llm-kimi',
    schema: JSON.parse(JSON.stringify(DeepSeekConfig.toJSON())) as unknown,
...
function harness(options: {
  ...
  const includeKimi = options.includeKimi === true
  const providers = options.provider === false
    ? []
    : [
      {
  ...
  const namespaces = options.settingsNamespace === false
    ? []
    : [
      deepSeekNamespace(apiKeyEnv),
      ...includeKimi ? [kimiNamespace('KIMI_API_KEY')] : [],
    ]
  const face = {
    llm: {
      providers: () => {
...
describe('DeepSeekOnboardingDialog', () => {
  ...
  it('loads a credential-only modal, inerts the product, and focuses the key', async () => {
    ...
    const provider = screen.getByLabelText<HTMLSelectElement>(en.provider)
    expect(provider.value).toBe('deepseek-official')
    expect([...provider.options].map(option => option.text)).toEqual(['DeepSeek'])
    const key = screen.getByLabelText<HTMLInputElement>(en.keyInput)
    await waitFor(() => { expect(document.activeElement).toBe(key) })
    expect(screen.queryByText(en.customized)).toBeNull()
  })

  it('lets the first-run user pick Kimi and store that provider key', async () => {
    const h = harness({ includeKimi: true })
    render(<DeepSeekOnboardingDialog {...h.props} />)
    ...
    fireEvent.change(provider, { target: { value: 'kimi-for-coding' } })
    expect(provider.value).toBe('kimi-for-coding')
    fireEvent.change(screen.getByLabelText(en.keyInput), { target: { value: 'sk-kimi' } })
    fireEvent.click(screen.getByRole('button', { name: en.onboardingSave }))
    await waitFor(() => {
      expect(h.set).toHaveBeenCalledWith({ ref: 'KIMI_API_KEY', value: 'sk-kimi' })
    })
  })
