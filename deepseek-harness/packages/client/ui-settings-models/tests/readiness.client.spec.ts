...
import { onboardingReadiness, onboardableRows, providerUsable } from '../src/client/store.ts'
...
function kimiRow(overrides: Partial<ProviderRow> = {}): ProviderRow {
  return {
    entry: {
...
describe('onboardingReadiness', () => {
  ...
  it('reports a missing writable effective credential', () => {
    ...
    expect(onboardingReadiness(state({ rows: [kimiRow()] }))).toEqual({ kind: 'credential-missing' })
    expect(onboardableRows(state({ rows: [row(), kimiRow()] })).map(item => item.entry.provider))
      .toEqual(['deepseek-official', 'kimi-for-coding'])
  })
  ...
  it('ends onboarding once any other registered provider can serve requests', () => {
    ...
    expect(onboardingReadiness(state({
      rows: [row({ entry: { ...row().entry, active: false } }), kimiRow()],
    }))).toEqual({ kind: 'credential-missing' })
  })
