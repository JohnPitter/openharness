...
import { en, es, pt, zh, type ThemeKey } from './locales.ts'
import {
...
export function apply(ctx: ClientContext): void {
  ...
  ctx.effect(() => ctx.locale.register(SETTINGS_NS, { zh, en, pt, es }), 'ui-theme: settings row dictionaries')
  ...
  // Embedded shells (the OpenHarness desktop window embeds this UI in an
  // iframe) mirror the titlebar palette from the resolved color scheme.
  if (typeof window !== 'undefined' && window.parent !== window) {
    const post = (scheme: 'light' | 'dark'): void => {
      window.parent.postMessage({ type: 'openharness:color-scheme', scheme }, '*')
    }
    ctx.on('theme/change', (snapshot) => { post(snapshot.active.colorScheme) })
    post(theme.getTheme().active.colorScheme)
  }
}
