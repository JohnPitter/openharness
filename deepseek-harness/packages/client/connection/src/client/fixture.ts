...
const PERMISSION_PRESETS: Record<string, { sandbox: string; approval: string; description: string }> = {
  ...
  'danger-full-access': { sandbox: 'danger-full-access', approval: 'always', description: 'Full file, process, and host access without approval prompts, including privileged operations.' },
}
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const api: ApiProxy = {
    sessions: {
      ...
      revise: request => ok(request, {
        revision: 1,
        eventSeq: logOf(request.payload.sessionId).at(-1)?.seq ?? 0,
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'session.revise': return this.api.sessions.revise(request, signal)
