...
export class FakeApiClient implements IApiClient {
  ...
  onRevise: (payload: unknown) => Promise<RpcResponse<{ revision: number; eventSeq: number; operationId: string }>> = () => Promise.resolve(ok({ revision: 1, eventSeq: 1, operationId: 'fake-operation' }))
  ...
  => Promise<RpcResponse<{ selected: ModelSelection; metering: 'tokens' | 'requests' }>> =
  ...
  onDelete: (payload: unknown) => Promise<RpcResponse<{ deleted: true; sessionIds: SessionId[] }>> =
    payload => Promise.resolve(ok({ deleted: true as const, sessionIds: [(payload as { sessionId: SessionId }).sessionId] }))
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    revise: (payload: unknown) => this.record('session.revise', payload, this.onRevise(payload)),
  ...
  readonly llm: IApiClient['llm'] = {
    ...
    accountUsage: payload => this.record('llm.accountUsage', payload, Promise.resolve(ok({ supported: false }))),
  }

  readonly usage: IApiClient['usage'] = {
    panel: payload => this.record('usage.panel', payload, Promise.resolve(ok({
      days: [],
      models: [],
