...
`WorkspaceListState.archivedSessionIds` 镜像 Host 的注册表级全局归档集合（一个按 Host 顺序的 `readonly SessionId[]`，仅在成员变化时才替换；需要 O(1) 查询的消费方自建临时 Set）。它是全快照状态：`workspace.list` 基线、`archiveSession`／`unarchiveSession` 一元回声和 `host/archived-sessions-changed` 帧各自安装完整集合。`WorkspaceRuntime.archiveSession(sessionId)` 通过 wire 归档，并在被归档 id 是当前会话时清空为 New Session 视图。从「已归档」打开会话会保持选中。其他标签页的 `host/archived-sessions-changed` 帧仅在当前 id 新加入集合时才清空当前会话。在 `workspace.list` 请求进行中安装的集合还会取代该过期基线携带的集合。各分组视图从活动分组中隐藏成员，尾部「已归档」分区列出它们，而会话行本身仍留在列表 store 中。`SessionRuntime.delete` 在 `session.delete` 之后永久丢掉被点名的会话及其 origin-subagent 后代。
