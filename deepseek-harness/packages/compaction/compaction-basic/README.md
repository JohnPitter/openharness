...
- **Convergence** — retry head-checkpoint compaction up to `compactionRetries`; reject a summary that does not shrink its source. When retries cannot return below threshold, keep the last successful replacement so a later step can continue.
- **Summarization** — a direct `llm/stream` call uses the configured provider/model pair and cap, falling back to the latest logged request target and then the agent target, without running the loop-only `agent/request` extension point. The call replays the conversation's own system prompt and shadowed-region messages verbatim, including image references, and appends the compaction instruction as the final user message; tool schemas are retained for region reconstruction but omitted from this summarizer request. Recorded milestone titles are appended under Critical Context so the checkpoint keeps the session index. The selected adapter must resolve or explicitly reject those images. It sets `GenerateOptions.purpose` to `compaction`, which adapters may forward as request attribution (the DeepSeek adapter sends `x-deepseek-harness-compact: 1`) without touching the model-visible body. Returned visible text enters the checkpoint; when visible text is empty, reasoning-only output is accepted as the fallback. A `max-tokens` finish is accepted when every required checkpoint heading is present in order (the last section may be cut short); an incomplete truncated body fails with `MAX_TOKENS`. Tool calls are excluded, and image output fails with `UNSUPPORTED_CONTENT` rather than disappearing.
...
- **Lifecycle** — all entry points share one bracket-first region transaction. It validates the range and live lock, appends `compaction/start` synchronously, prepares and awaits the summary, revalidates, appends `compaction/summary` plus the replacement, and makes exactly one closing attempt. Automatic and explicit-region calls require a numeric open-turn owner and whole-surface stability; the serial `agent/pre-step` listener checks pressure before request derivation, while canonical provider overflow enters through `agent/request-error` and authorizes retry only after durable surface progress. `compactNow()` reserves idle admission, prunes oversized tool results before it opens the standalone `turn: null` bracket, accepts append-only context outside its selected span, flushes every closed attempt, and releases admission in `finally`.
- **Overflow recovery** — provider-confirmed overflow bypasses the pressure threshold. Manual and overflow compaction resolve the recorded request window and cap the span to `min(summarizerSpanCeiling, floor(window / 2) - output - summarizerEnvelopeReserve)`, reserving output and envelope inside a defensive half-window and then applying the configured (default 65,536-token) latency ceiling so a 1M route does not send a half-window request. Sessions without a recorded window use the documented `128_000` safety fallback; invalid or null windows fail closed with a zero budget. A zero safe budget selects no range and makes no LLM request. Pressure uses the same envelope-aware cap when it is positive, so a 75% occupancy session on a production window does not send the whole surface to the summarizer; a zero envelope-aware budget leaves pressure uncapped. Manual `/compact` prunes oversized tool results before range selection. If the summarizer reports canonical `CONTEXT_WINDOW_EXCEEDED` or `MAX_TOKENS`, the same transaction halves its span budget and retries up to three total attempts, reselecting balanced boundaries without mutating the surface or emitting failed-attempt events. Other errors and cancellation are not retried. No replacement, an exhausted target-specific cap, or an unknown/noncanonical error preserves the original provider failure.
...
| `summarizerSpanCeiling` | no (default `65536`) | Absolute latency-safe ceiling, in tokens, on the conversation prefix a summarization call replays; lowering it shrinks every compaction's auxiliary call at the cost of a shorter, coarser checkpoint on very large windows. |
| `summarizerEnvelopeReserve` | no (default `16384`) | Reserve, in tokens, subtracted for the summarizer's own system prompt and instruction before halving the routed model's context window. |
...
When a settings provider is loaded, the first engine that can `ctx.get('settings')` registers namespace `compaction-basic` with `auto` (default `true`) and `thresholdPercent` (`25`, `50`, `75`, or `100`; default `75`, the closest allowed fraction to Config `thresholdRatio` `0.8`). It never reads `ctx.settings`, so an agent-preset fiber that did not inject settings still mounts. Pressure uses `thresholdPercent / 100` in place of the routed Config `thresholdRatio`. Turning `auto` off skips both automatic listeners at the next step or overflow; `/compact` and `compactNow()` stay available. Compositions without settings keep the plugin Config, including `thresholdRatio: 0.8`.
...
Model-free pruning can avoid the auxiliary call entirely; otherwise it reduces that call's transcript before the summary replaces an older range. Manual and overflow spans reserve the output cap and `summarizerEnvelopeReserve` inside a defensive half of the recorded window, then cap the priced span at `summarizerSpanCeiling` (default 65,536) tokens; a zero safe budget makes no LLM request. Pressure remains threshold-driven and uses that same positive cap. The replacement reduces future input history rather than appending a second copy. A summary remains until a later compaction replaces it, while an indivisible non-tool unit can still exceed the budget.
...
The summarization model receives the conversation replayed verbatim — the same system prompt and messages the last routed request sent for the shadowed region — followed by one final user message: the compaction instruction below. Recorded tool schemas are omitted from this request. When visible output is empty, reasoning-only output is accepted as the summary fallback; otherwise returned visible text is stored.
