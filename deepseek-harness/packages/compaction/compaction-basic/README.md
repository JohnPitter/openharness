...
- **Overflow recovery** — provider-confirmed overflow bypasses the pressure threshold. When the adapter advertises a recorded window, manual and overflow compaction cap the span to `floor(window / 2) - output - envelope`, reserving output and envelope inside a defensive half-window; a zero safe budget selects no range and makes no LLM request. Pressure remains threshold-driven and uncapped. Without a recorded window, overflow still attempts one maximal balanced head reduction. Retry is authorized whenever `surface.replaceGeneration` advances, including when pruning lands before later summary work throws. No replacement, an exhausted target-specific cap, cancellation, or an unknown/noncanonical error preserves the original provider failure.
