...
Model-free pruning can avoid the auxiliary call entirely; otherwise it reduces that call's transcript before the summary replaces an older range. Manual and overflow spans reserve the output cap and envelope inside a defensive half of the recorded window, then cap the priced span at 65,536 tokens; a zero safe budget makes no LLM request. Pressure remains threshold-driven and uses that same positive cap. The replacement reduces future input history rather than appending a second copy. A summary remains until a later compaction replaces it, while an indivisible non-tool unit can still exceed the budget.
