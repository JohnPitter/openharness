...
- **溢出恢复**：提供方已确认的溢出会绕过压力阈值。手动和溢出压缩会解析已记录的请求窗口，并将区间限制为 `floor(window / 2) - output - envelope`，在防御性的一半窗口内预留输出与 envelope。没有记录窗口的会话使用文档规定的 `128_000` 安全回退；空值或无效窗口会以零预算安全失败。安全预算为零时不选择区间，也不会发起 LLM 请求。压力仍由阈值驱动且不设此上限。如果摘要器报告规范的 `CONTEXT_WINDOW_EXCEEDED`，同一事务会将区间预算减半，最多共尝试三次，重新选择平衡边界，不修改表层，也不产生失败尝试的事件。其他错误和取消不会重试。如果没有替换、目标特定上限已耗尽，或遇到未知／非规范错误，则保留原始提供方失败。
