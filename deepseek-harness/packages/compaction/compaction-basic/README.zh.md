...
- **不依赖模型的剪枝**：在压力或规范溢出符合条件后，可选的 [`ctx.toolResultPruner`](../compaction-tool-result-pruner/README.zh.md) 服务会在选择范围之前改写超大工具结果。Compact-basic 通过 `ctx.tokenMeter` 重新测量；如果压力已回到安全范围，就跳过摘要，否则对已剪枝的表层进行摘要。低于压力的步骤检查绝不剪枝。按请求计费的路由仍会跑压力摘要；实时覆盖是通用设置里的 `auto` / `thresholdPercent`。该行为由[按请求路由的压力压缩 Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-24-request-route-pressure-compact.zh.md) 拥有。
...
- **收敛**：最多按 `compactionRetries` 重试头部检查点压缩；拒绝不能缩小源内容的摘要。若重试仍无法回到阈值以下，则保留最近一次成功替换，供后续步骤继续。
- **摘要**：直接 `llm/stream` 调用使用已配置的提供方／模型对与上限，回退到最新已记录请求目标，然后再回退到 agent（智能体）目标，而不运行仅用于 agent loop 的 `agent/request` 扩展点。该调用会逐字回放会话自身的系统提示词与已遮蔽区域消息（包括图片引用），并将压缩指令作为最后一条 user 消息追加，从而复用提供方的热前缀 cache，而非使它失效；工具 schema 仅为区域重建保留，不会发送到摘要请求。已记录的里程碑标题会追加到 Critical Context 下，使检查点保留会话索引。所选适配器必须解析或明确拒绝这些图片。它将 `GenerateOptions.purpose` 设为 `compaction`，适配器可将其作为请求归因转发（DeepSeek 适配器发送 `x-deepseek-harness-compact: 1`），但不会触碰模型可见的请求体。有可见文本时保存该文本；可见文本为空时接受仅推理输出作为回退。`max-tokens` 结束在按顺序包含全部检查点标题时被接受（最后一节可以写短）；不完整的截断正文以 `MAX_TOKENS` 失败。工具调用会被排除；图片输出会以 `UNSUPPORTED_CONTENT` 失败，而不是消失。
- **框定**：替换 user 消息使用 `<compacted-summary>` 标签标记已建立的检查点上下文。原始摘要保留在 `compaction/summary` 事件上，后续自动周期会合并之前的检查点。
- **生命周期**：所有入口点共享一个先记录标记的区域事务。它会验证范围与活动锁，同步追加 `compaction/start`，准备并等待摘要，重新验证，再追加 `compaction/summary` 和替换，最后恰好进行一次闭合尝试。自动调用和显式范围调用要求数字标识的开放轮次归属，并要求整个表层保持稳定；串行 `agent/pre-step` listener 会在派生请求之前检查压力，而规范提供方溢出则经由 `agent/request-error` 进入，并且只在表层取得持久进展后才允许重试。`compactNow()` 会预留空闲接纳，在打开独立的 `turn: null` 括号之前剪枝超大工具结果，允许所选 span 之外追加仅追加上下文，flush 每次已闭合尝试，并在 `finally` 中释放接纳预留。
- **溢出恢复**：提供方已确认的溢出会绕过压力阈值。手动和溢出压缩会解析已记录的请求窗口，并将区间限制为 `min(summarizerSpanCeiling, floor(window / 2) - output - summarizerEnvelopeReserve)`，在防御性的一半窗口内预留输出与 envelope，再套用可配置（默认 65,536 token）的延迟上限，避免 1M 路由发送半窗请求。没有记录窗口的会话使用文档规定的 `128_000` 安全回退；空值或无效窗口会以零预算安全失败。安全预算为零时不选择区间，也不会发起 LLM 请求。压力在 envelope 感知上限为正时使用该上限，因此占用到 75% 的生产窗口会话不会把整段表层发给摘要器；该预算为零时压力不设上限。手动 `/compact` 在选择区间前会剪枝超大工具结果。如果摘要器报告规范的 `CONTEXT_WINDOW_EXCEEDED` 或 `MAX_TOKENS`，同一事务会将区间预算减半，最多共尝试三次，重新选择平衡边界，不修改表层，也不产生失败尝试的事件。其他错误和取消不会重试。如果没有替换、目标特定上限已耗尽，或遇到未知／非规范错误，则保留原始提供方失败。
...
| `summarizerSpanCeiling` | 否（默认 `65536`） | 摘要调用回放会话前缀的绝对延迟安全上限（token 数）；调低它会缩小每次压缩的辅助调用，代价是超大窗口下检查点更短、更粗略。 |
| `summarizerEnvelopeReserve` | 否（默认 `16384`） | 在对已路由模型上下文窗口取半之前，为摘要器自身系统提示与指令预留的 token 数。 |
| `modelPolicies` | 否（默认 `[]`） | 精确的 `{ provider, model, ...partialPolicy }` 覆盖；匹配使用两个字段，不依赖 `listModels()`。 |
| `auto` | 否（默认 `true`） | 注册步骤边界压力与溢出恢复 listener。设为 `false` 则仅手动执行。已注册的 `compaction-basic` settings 段会在事件时覆盖此项。 |
...
不依赖模型的剪枝可以完全避免辅助调用；否则它会在摘要替换较早范围之前缩减该调用的 transcript（文本记录）。手动和溢出区间会在记录窗口的一半防御范围内预留输出上限与 `summarizerEnvelopeReserve`，再将计价区间封顶为 `summarizerSpanCeiling`（默认 65,536）tokens；安全预算为零时不会发起 LLM 请求。压力仍由阈值驱动，并使用同一正向上限。替换会缩减未来输入历史，而非追加第二份副本。摘要会保留到后续压缩将其替换，但不可分的非工具单元仍可能超出预算。
...
摘要模型会接收逐字回放的会话：与上次已路由请求为已遮蔽区域发送的相同系统提示词与消息，后面跟随一条最终 user 消息，即下方压缩指令；记录的工具 schema 不会发送到该请求。可见文本为空时，接受仅推理输出作为摘要回退；否则存储返回的可见文本。
