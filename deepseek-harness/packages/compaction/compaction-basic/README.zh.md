...
- **溢出恢复**：提供方已确认的溢出会绕过压力阈值。最新记录的窗口存在时，手动和溢出压缩会将区间限制为 `floor(window / 2) - output - envelope`，在防御性的一半窗口内预留输出与 envelope；安全预算为零时不选择区间，也不会发起 LLM 请求。压力仍由阈值驱动且不设此上限。没有记录窗口时仍尝试一次最大平衡头部缩减。只要 `surface.replaceGeneration` 前进，就允许重试，包括剪枝在后续摘要工作抛出异常前已落地的情况。如果没有替换、目标特定上限已耗尽、已取消，或遇到未知／非规范错误，则保留原始提供方失败。
