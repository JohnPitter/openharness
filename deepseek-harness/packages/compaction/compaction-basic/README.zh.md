...
- **溢出恢复**：提供方已确认的溢出会绕过压力阈值。手动和溢出压缩会解析已记录的请求窗口，并将区间限制为 `min(SUMMARIZER_SPAN_CEILING, floor(window / 2) - output - envelope)`，在防御性的一半窗口内预留输出与 envelope，再套用 65,536 token 的延迟上限，避免 1M 路由发送半窗请求。没有记录窗口的会话使用文档规定的 `128_000` 安全回退；空值或无效窗口会以零预算安全失败。安全预算为零时不选择区间，也不会发起 LLM 请求。压力在 envelope 感知上限为正时使用该上限，因此占用到 75% 的生产窗口会话不会把整段表层发给摘要器；该预算为零时压力不设上限。手动 `/compact` 在选择区间前会剪枝超大工具结果。如果摘要器报告规范的 `CONTEXT_WINDOW_EXCEEDED` 或 `MAX_TOKENS`，同一事务会将区间预算减半，最多共尝试三次，重新选择平衡边界，不修改表层，也不产生失败尝试的事件。其他错误和取消不会重试。如果没有替换、目标特定上限已耗尽，或遇到未知／非规范错误，则保留原始提供方失败。
...
不依赖模型的剪枝可以完全避免辅助调用；否则它会在摘要替换较早范围之前缩减该调用的 transcript（文本记录）。手动和溢出区间会在记录窗口的一半防御范围内预留输出上限与 envelope，再将计价区间封顶为 65,536 tokens；安全预算为零时不会发起 LLM 请求。压力仍由阈值驱动，并使用同一正向上限。替换会缩减未来输入历史，而非追加第二份副本。摘要会保留到后续压缩将其替换，但不可分的非工具单元仍可能超出预算。
