...
/** Canonical provider-neutral code for a request that has no credential at all. */
export const MISSING_CREDENTIAL_CODE = 'MISSING_CREDENTIAL'

/**
 * Whether `error` is the adapter-neutral missing-credential failure.
 * Catalog builders hide a route from the picker on this code without treating
 * absence as a catalog load error.
 */
export function isMissingCredential(error: unknown): boolean {
  return error instanceof HarnessError && error.code === MISSING_CREDENTIAL_CODE
}
