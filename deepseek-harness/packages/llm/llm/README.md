...
- `ctx.llm.providerMetering(provider: string): LlmMetering` Return the adapter-declared charged unit for one registered route, or `tokens` when the route is unregistered or undeclared. Consumers must not infer this from the provider id.
