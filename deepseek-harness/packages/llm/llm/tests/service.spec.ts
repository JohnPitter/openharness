...
describe('LlmRuntime', () => {
  ...
  it('honours an adapter that withholds its models from the picker catalog', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    ctx.llm.registerAdapter(['hidden'], new class extends ScriptedAdapter {
      override advertiseModels(): Promise<boolean> {
        return Promise.resolve(false)
      }
    }(SCRIPT))
    await expect(ctx.llm.advertiseModels('hidden')).resolves.toBe(false)
  ...
  it('defaults adapters to their route name and an empty advisory model list', async () => {
    ...
    await expect(ctx.llm.advertiseModels('plain')).resolves.toBe(true)
    await expect(ctx.llm.listModels('missing')).rejects.toMatchObject({ code: 'NO_ADAPTER' })
    await expect(ctx.llm.advertiseModels('missing')).rejects.toMatchObject({ code: 'NO_ADAPTER' })
