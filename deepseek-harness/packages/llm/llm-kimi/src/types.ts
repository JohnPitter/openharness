/**
 * Kimi for Code chat-completions wire format (OpenAI-compatible). Types only.
 *
 * Source of truth: MoonshotAI/kimi-cli and kosong sources (2026-08).
 *
 * @module dsh-llm-kimi/types
 */
...
export interface WireRequest {
  ...
  /** Thinking-mode toggle. Used by K2-family models; K3 uses {@link reasoning_effort} instead. */
  thinking?: { type: 'enabled' | 'disabled' }
  /**
   * K3 reasoning depth (`low` / `high` / `max`). Coding-API default is `high`.
   * Must not be sent together with {@link thinking} on K3.
   */
  reasoning_effort?: 'low' | 'high' | 'max'
  /** Prompt-cache affinity key; kimi-cli sets it to the session id. */
 ...
 * `mapUsage` subtracts them to keep the harness convention of disjoint
 * counts. `prompt_tokens_details.cached_tokens` is the OpenAI-compat
 * spelling of the hit count.
 */
