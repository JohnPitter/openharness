...
function harnessApiKeyAuth(name: string): ApiKeyAuth {
  return {
    ...
    // `resolve` must not fail a request that will receive the harness key as
    // a stream override. `check` is the availability probe, and without a
    // stored key this route is not configured — otherwise an OAuth-only
    // catalog (Codex) looks signed-in to the picker with no credential.
    check: ({ credential }) => Promise.resolve(
      credential?.key !== undefined && credential.key.length > 0
        ? { source: name, type: 'api_key' as const }
        : undefined,
    ),
  }
}
...
export interface ProviderSpec {
  ...
  displayName: string
  /** Installed catalog to reuse when it is not {@link provider}. */
  catalog?: string
...
export function buildProvider(spec: ProviderSpec): Provider {
  const catalog = catalogProvider(spec.catalog ?? spec.provider)
