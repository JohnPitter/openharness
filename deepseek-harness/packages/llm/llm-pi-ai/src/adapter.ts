...
export class PiAiAdapter extends LlmAdapter {
  ...
  /** Route ids whose catalog provider declares an OAuth login method. */
  oauthRouteIds(): string[] {
    return [...this.current().profiles.entries()]
      .filter(([, profile]) => profile.piProvider.auth.oauth !== undefined)
  ...
  login(provider: string, interaction: AuthInteraction): Promise<Credential> {
    ...
    return this.current().models.login(provider, 'oauth', interaction)
  }

  /** Drop the stored OAuth credential for one route. */
  logout(provider: string): Promise<void> {
    this.profileOf(this.current(), provider)
    return this.current().models.logout(provider)
  }

  /** Whether the route currently has complete auth (API key, OAuth, or ambient). */
  ...
  override async advertiseModels(provider: string): Promise<boolean> {
    const snapshot = this.current()
    ...
    // Do not ask Models.checkAuth: the harness api-key stub on OAuth-only
    // catalog routes (Codex) reports configured with no key. The picker
    // follows the harness credential seam and the durable auth store.
    const stored = await this.config.auth.credentials.read(provider)
    if (stored?.type === 'oauth') return true
    return profile.apiKeyEnv === undefined
  }

  /**
   * Weekly / 5-hour coding-plan windows. Claude Code, Codex, and GLM publish
  ...
  private async bearerToken(
    ...
    throw new LlmError(
      `llm-pi-ai: no credential for provider route "${provider}"`,
      'MISSING_CREDENTIAL',
    )
  }
  ...
  private async * streamWithSnapshot(
    ...
    if (
      ...
      const auth = await snapshot.models.checkAuth(options.provider)
      if (auth === undefined) {
        throw new LlmError(
          `llm-pi-ai: no credential for provider route "${options.provider}"; sign in with OAuth in`
          + ` Settings → Models or store ${profile.apiKeyEnv} as an API key`,
          'MISSING_CREDENTIAL',
        )
      }
    }
