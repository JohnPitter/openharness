 ...
 * @param catalogIdOf - maps a route key to the installed catalog vendor it
 *   inherits (`claude-code` → `anthropic`). Absent, the route key is the
 *   catalog id.
...
export async function discoverModels(
  ...
  catalogIdOf?: (provider: string) => string | undefined,
): Promise<readonly LlmDiscoveredModel[]> {
  // A catalog route already has its answer, and a better one: the installed
  // entries carry context windows and output caps no listing endpoint reports.
  // An alias (`claude-code` → `anthropic`) looks up the inherited vendor, not
  // the route key — pi-ai does not ship a vendor named `claude-code`.
  if (request.provider !== undefined) {
    const catalogId = catalogIdOf?.(request.provider) ?? request.provider
    const installed = catalogModels(catalogId)
    if (installed.size > 0) {
  ...
  // and its credential resolver remains lazy so a typed key cannot fail over
  // a stored credential it supersedes. A route may still authenticate through a
