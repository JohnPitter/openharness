...
export interface RouteCatalogRequest {
  ...
  provider: string
  /**
   * Installed catalog to inherit when it is not this route's key. A second
   * billed path (Claude Code OAuth beside the Console API key) needs its own
   * route id but the same model table, endpoint, and compat as the vendor
   * catalog. Absence inherits under the route key, as before.
   */
  catalog?: string
...
export function resolveRouteModels(request: RouteCatalogRequest): RouteCatalog {
  ...
  const catalogId = request.catalog ?? provider
  if (request.catalog !== undefined && request.catalog.length === 0) {
    invalid(provider, 'has an empty catalog; omit the field to inherit under this route key, or name an installed provider')
  }
