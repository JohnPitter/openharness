...
询问读 `openai-completions`、`openai-responses` 与 `anthropic-messages`。OpenAI 协议在 `GET {baseURL}/models` 上用 bearer 认证；Anthropic Messages 走原生 `GET /v1/models?limit=1000`，带 `x-api-key` 与 `anthropic-version`；其列表 URL 接受带或不带末尾 `/v1` 的 API 根，因为网关文档两种拼写都会出现——只有这条列表 URL 会归一化该段，因此模型请求仍使用配置的 `baseURL`。Azure 尽管出身 OpenAI 也被排除——它用 `api-key` 标头认证并要求 `api-version` 查询参数——Codex 则走 OAuth；其余协议一律以 `DISCOVERY_UNSUPPORTED` 回答，让界面回退到手工填写，而不是把认证失败报成一个没有模型的提供方。`baseURL` 按前缀而非待解析 URL 处理，因此 `https://gateway.example/openai/v1` 这类部署路径会保留其路径段。已配置的具名路由在 Host 内提供其已存储凭据与 profile `headers`，因此经 `settings.yaml` 或 Cordis 配置的部署标头会到达模型询问，而不会变成询问请求或 Models 页上的字段；表单里键入的密钥仍优先于已存储凭据。

解析器接受标准 `data` 数组或充实的 `models` 映射，并归一化每个候选的 id、显示名称、上下文窗口与输出 token 上限。Anthropic 的 `max_input_tokens` 与 `max_tokens` 填入同一套容量字段；映射键始终是请求 id，即使条目里另有规范 id；原始值映射属性会被忽略；缺失的显示名称回退到该请求 id。没有可用 id 的条目会被跳过而不是让整份列表失败。回复在四兆字节上限下读取，且上限落在实际收到的字节上——端点是用户自己填的 URL，因此会先看声明长度，但绝不把它当作边界。端点不可达、凭据被拒、响应非 JSON、以及响应既没有 `data` 数组也没有 `models` 对象，都会以 `DISCOVERY_FAILED` 失败，消息点名端点；仅当 401 或 403 时才点名凭据。读取响应体期间被取消会呈现为 `ABORTED`，与请求发出之前被取消一致。
