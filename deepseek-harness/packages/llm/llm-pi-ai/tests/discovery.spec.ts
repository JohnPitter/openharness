...
describe('catalog-route model discovery', () => {
  ...
  it('answers an inherited catalog alias from the named vendor, with no network call', async () => {
    const server = await listingServer({ body: JSON.stringify({ data: [{ id: 'from-the-endpoint' }] }) })
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmPiAi, {
    ...
    const models = await ctx.llm.discoverModels('llm-pi-ai', { provider: 'claude-code', baseURL: server.url })
    expect(models.map(model => model.id).sort())
...
describe('draft-provider model discovery', () => {
  ...
  it('reads an OpenAI-compatible listing and keeps the camelCase and mixed capacity fields it discloses', async () => {
    const server = await listingServer({
      body: JSON.stringify({
        data: [
  ...
  it('authenticates configured routes the draft cannot supply a key for', async () => {
    ...
    // and read as a wrong key. Deployment headers live on the profile, not the
    // discovery request, so a settings.yaml `headers` entry still reaches GET
    // /models.
    const server = await listingServer({ body: JSON.stringify({ data: [{ id: 'm' }] }) })
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
