...
describe('provider profile lifecycle', () => {
  ...
  it('withholds named-key routes from the picker until a credential exists', async () => {
    vi.stubEnv('PI_CUSTOM_REF_KEY', '')
    const server = await mockServer([])
    const ctx = await harness(server.url, { apiKey: undefined, apiKeyEnv: 'PI_CUSTOM_REF_KEY' })
    await expect(ctx.llm.advertiseModels('deepseek')).resolves.toBe(false)
    await expect(ctx.llm.listModels('deepseek')).resolves.not.toHaveLength(0)
  })

  it('advertises a named-key route once the credential resolves', async () => {
    const server = await mockServer([])
    const ctx = await harness(server.url)
    await expect(ctx.llm.advertiseModels('deepseek')).resolves.toBe(true)
  })

  it('advertises a route that names no credential (provider-native auth)', async () => {
    const adapter = adapterOf({ openai: {} }, undefined)
    await expect(adapter.advertiseModels('openai')).resolves.toBe(true)
  })

  it('withholds an OAuth coding-plan route until signed in or a key is pasted', async () => {
    const unsigned = new PiAiAdapter({
      profiles: () => resolveProfiles({
        'claude-code': { apiKeyEnv: 'CLAUDE_CODE_OAUTH_TOKEN', catalog: 'anthropic' },
      }),
      resolveApiKey: () => Promise.resolve(undefined),
      auth: memoryAuth(),
    })
    await expect(unsigned.advertiseModels('claude-code')).resolves.toBe(false)
    const keyed = adapterOf({
      'claude-code': { apiKeyEnv: 'CLAUDE_CODE_OAUTH_TOKEN', catalog: 'anthropic' },
    }, 'sk-ant-oat-test')
    await expect(keyed.advertiseModels('claude-code')).resolves.toBe(true)
  })

  it('does not advertise Codex from the dummy harness api-key method', async () => {
    // namesCredential adds harness api-key auth so a pasted token can ride
    // the stream override; that stub must not make the picker think Codex
    // is signed in.
    const unsigned = new PiAiAdapter({
      profiles: () => resolveProfiles({
        'openai-codex': { apiKeyEnv: 'CODEX_ACCESS_TOKEN' },
      }),
      resolveApiKey: () => Promise.resolve(undefined),
      auth: memoryAuth(),
    })
    await expect(unsigned.advertiseModels('openai-codex')).resolves.toBe(false)
    await expect(unsigned.checkAuth('openai-codex')).resolves.toBeUndefined()
  })

  it('advertises Codex when the OAuth store holds a token', async () => {
    const unsigned = new PiAiAdapter({
      profiles: () => resolveProfiles({
        'openai-codex': { apiKeyEnv: 'CODEX_ACCESS_TOKEN' },
      }),
    ...
    await expect(unsigned.advertiseModels('openai-codex')).resolves.toBe(true)
  })
...
describe('PiAiAdapter accountUsage', () => {
  function jsonFetch(url: string, body: unknown, status = 200): void {
  ...
  it('reads Claude Code five-hour and weekly utilization', async () => {
    jsonFetch('https://api.anthropic.com/api/oauth/usage', {
    ...
    const usage = await adapter.accountUsage('claude-code')
    expect(usage?.windows.map(window => window.id)).toEqual(['weekly', 'rate'])
    const fetch = vi.mocked(globalThis.fetch)
    const headers = new Headers(fetch.mock.calls[0]?.[1]?.headers)
    expect(headers.get('authorization')).toBe('Bearer sk-ant-oat-test')
    expect(headers.get('user-agent')?.startsWith('claude-code/')).toBe(true)
  })

  it('reads Codex rate-limit windows from a JWT', async () => {
    const payload = Buffer.from(JSON.stringify({ chatgpt_account_id: 'acct_1' })).toString('base64url')
    const token = `h.${payload}.sig`
    jsonFetch('https://chatgpt.com/backend-api/wham/usage', {
      plan_type: 'pro',
      rate_limit: {
        primary_window: { used_percent: 25, limit_window_seconds: 18_000, reset_at: 1_777_534_802 },
        secondary_window: { used_percent: 9, limit_window_seconds: 604_800, reset_at: 1_777_969_707 },
      },
    })
    const adapter = adapterOf({ 'openai-codex': { apiKeyEnv: 'CODEX_ACCESS_TOKEN' } }, token)
  ...
  it('reads GLM coding-plan quota from the monitor host', async () => {
    jsonFetch('https://api.z.ai/api/monitor/usage/quota/limit', {
      data: {
        planName: 'Lite',
        limits: [
    ...
    const adapter = adapterOf({
      zai: { apiKeyEnv: 'ZAI_API_KEY', baseURL: 'https://api.z.ai/api/coding/paas/v4' },
    }, 'zai-key')
    const usage = await adapter.accountUsage('zai')
    expect(usage?.plan).toBe('Lite')
    expect(usage?.windows.map(window => window.id)).toEqual(['rate', 'weekly'])
  })

  it('has no quota surface on pay-per-token catalog routes', async () => {
    const adapter = adapterOf({ openai: { apiKeyEnv: 'OPENAI_API_KEY' } })
