...
describe('serializeRequest', () => {
  ...
  it('disables thinking for compaction requests', () => {
    const wire = serializeRequest(
      request({
        messages: history,
        purpose: 'compaction',
  ...
  it('maps low onto enabled thinking with wire reasoning_effort', () => {
    const wire = serializeRequest(
  ...
  it('maps medium and xhigh onto high as Chat Completions aliases', () => {
    const medium = serializeRequest(
      request({ messages: history, reasoningEffort: ReasoningEffortId('medium') }),
    )
    expect(medium.thinking).toEqual({ type: 'enabled' })
    expect(medium.reasoning_effort).toBe('high')
    const xhigh = serializeRequest(
      request({ messages: history, reasoningEffort: ReasoningEffortId('xhigh') }),
    )
