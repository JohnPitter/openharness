...
describe('goal tool state transitions', () => {
  it('reads null, then edits, pauses, and resumes by exact revision in one human turn', async () => {
    ...
    const resumed = ctx.goals.resume(root.agent, {
      id: GoalId(goal['id'] as string),
      revision: goal['revision'] as number,
    })
    expect(resumed).toMatchObject({ phase: 'active', revision: 4 })
  })

  it('rejects update_goal resume of a durable paused goal', async () => {
    const { ctx, root } = await harness()
    openTurn(root, { kind: 'user' })
    const created = ctx.goals.create(root.agent, { objective: 'user paused this' })
    const paused = ctx.goals.pause(root.agent, created)
    const refused = await execute(ctx, 'update_goal', {
      goal_id: paused.id, revision: paused.revision, action: 'resume',
    }, root.agent)
    expect(refused.error?.info?.code).toBe('GOAL_TOOL_RESUME_PAUSED')
    expect(ctx.goals.get(root.agent)).toMatchObject({ phase: 'paused', revision: paused.revision })
  })
  ...
  it('accepts only empty fillers in fields unused by the selected action', async () => {
    ...
    expect(goal).toMatchObject({ phase: 'active', objective: 'edited' })
