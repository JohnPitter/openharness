...
export interface ApiRemoteAgentOptions {
  ...
  /**
   * Receive the owned handle of a cold resume this resolver just published.
   * The Host keeps the handle so `session.delete` can dispose an identity it
   * resumed; live Agents found already registered are not reported.
   * @param handle - the resume handle this resolver now owns.
   */
  readonly onHandle?: (handle: AgentHandle) => void
}
