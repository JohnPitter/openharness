...
| `delete(id, signal?): Promise<void>` | Drop one session from coordinator state and durable storage. A live owner must dispose first; an in-flight retirement drains before the artifact is removed. Absent identities resolve without writing. |
...
| `deleteStored(id, signal?)` | Durably remove one materialized session. Absent ids resolve without writing. Events, metadata, and any per-session artifact go together. |
