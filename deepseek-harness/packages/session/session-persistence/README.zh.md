...
| `delete(id, signal?): Promise<void>` | 从协调器状态和持久存储中丢掉一个会话。活动所有者必须先 dispose；进行中的退役会在移除产物前排空。缺失身份直接完成而不写入。 |
...
| `deleteStored(id, signal?)` | 持久移除一个已实体化会话。缺失 id 直接完成而不写入。事件、元数据和任何每会话产物一并去掉。 |
