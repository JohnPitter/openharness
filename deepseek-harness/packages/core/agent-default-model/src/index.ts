...
/** Settings namespace carrying the worker model for Workflow-mode subagents. */
export const WORKFLOW_WORKER_MODEL_SETTINGS_NAMESPACE = settingsNamespace('workflow-worker-model')

/** Preset id whose children use {@link AgentDefaultModelConfig.currentWorkerSelection}. */
export const WORKFLOW_PRESET_ID = 'workflow'
...
export class AgentDefaultModelConfig extends Service {
  ...
  private workerSource: () => AgentDefaultModelSettings
  ...
  constructor(ctx: Context, config: Config) {
    ...
    const workerEntry: AgentDefaultModelSettings = { provider: '', model: '' }
    this.workerSource = () => workerEntry
    installSettingsSection(
      ctx,
      WORKFLOW_WORKER_MODEL_SETTINGS_NAMESPACE,
      AGENT_DEFAULT_MODEL_SETTINGS_SCHEMA,
      workerEntry,
      {
        setSource: (current) => { this.workerSource = current },
        onChange: () => {},
      },
    )
  }
  ...
  currentWorkerSelection(): ModelSelection | undefined {
    const stored = this.workerSource()
    if (stored.provider === '' || stored.model === '') return undefined
  ...
  async saveWorkerSelection(next: ModelSelection): Promise<void> {
    await this.ctx.get('settings')?.replace(WORKFLOW_WORKER_MODEL_SETTINGS_NAMESPACE, {
      provider: next.provider,
      model: next.model,
      ...next.reasoningEffort === undefined ? {} : { reasoningEffort: String(next.reasoningEffort) },
    })
