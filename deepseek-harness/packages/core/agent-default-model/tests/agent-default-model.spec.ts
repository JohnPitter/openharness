...
describe('AgentDefaultModelConfig', () => {
  ...
  it('stores a worker model independently of the Agent default', async () => {
    const bench = await boot()
    expect(bench.defaultModel.currentWorkerSelection()).toBeUndefined()
    await bench.defaultModel.saveWorkerSelection({
      provider: 'acme-gateway', model: 'acme-small', reasoningEffort: ReasoningEffortId('off'),
    })
    expect(bench.defaultModel.currentWorkerSelection()).toEqual({
      provider: 'acme-gateway', model: 'acme-small', reasoningEffort: 'off',
    })
    expect(bench.defaultModel.currentSelection()).toEqual({
      provider: 'deepseek-official', model: 'deepseek-v4-flash',
    })
