...
当部署组装了 agent-preset roster 时，快照还携带 `agentPresets`：每个 preset 一组扁平化的组合行。已有常驻挂载的从该挂载回答；尚未挂载的从组合文件回答且不挂载。没有 roster 时该字段缺席。
...
- **仅在有 roster 时出现 preset** —— 未组装 `dsh-agent-presets` 的部署只提供 Loader 条目；`agentPresets` 字段缺席而不是空数组。
