...
When an agent-preset roster is composed, the snapshot also carries `agentPresets`: one group per preset with flattened composition rows. A live standing mount answers from that mount; an unmounted preset answers from its composition file without mounting. Without a roster the field is absent.
