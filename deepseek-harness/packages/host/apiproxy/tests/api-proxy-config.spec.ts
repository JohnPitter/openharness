...
import type { GenerateOptions, LlmAccountUsage, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@deepseek-ai/dsh-llm'
...
class QuotaAdapter extends LlmAdapter {
  constructor(private readonly fail?: 'MISSING_CREDENTIAL') {
    super()
  }

  override async accountUsage(): Promise<LlmAccountUsage | undefined> {
    if (this.fail === 'MISSING_CREDENTIAL') {
      throw new LlmError('no API key', 'MISSING_CREDENTIAL')
    }
    return {
      plan: 'Moderato',
      windows: [
...
describe('llm.accountUsage', () => {
  it('reports unsupported when the adapter has no quota surface', async () => {
    const ctx = await harness()
    ctx.llm.registerAdapter(['deepseek-official'], new CatalogAdapter('DeepSeek', ['deepseek-v4-flash']))
    const api = createApiProxy(ctx, DEFAULTS)
    expect(expectOk(await api.llm.accountUsage(request({ provider: 'deepseek-official' })))).toEqual({
      supported: false,
    })
  })

  it('reports unsupported for an unregistered route', async () => {
    const ctx = await harness()
    const api = createApiProxy(ctx, DEFAULTS)
    expect(expectOk(await api.llm.accountUsage(request({ provider: 'kimi-for-coding' })))).toEqual({
      supported: false,
    })
  })

  it('returns quota windows from the owning adapter', async () => {
    const ctx = await harness()
    ctx.llm.registerAdapter(['kimi-for-coding'], new QuotaAdapter())
    const api = createApiProxy(ctx, DEFAULTS)
    expect(expectOk(await api.llm.accountUsage(request({ provider: 'kimi-for-coding' })))).toEqual({
      supported: true,
      plan: 'Moderato',
  ...
  it('keeps credential failures on the value instead of failing the RPC', async () => {
    const ctx = await harness()
    ctx.llm.registerAdapter(['kimi-for-coding'], new QuotaAdapter('MISSING_CREDENTIAL'))
    const api = createApiProxy(ctx, DEFAULTS)
    expect(expectOk(await api.llm.accountUsage(request({ provider: 'kimi-for-coding' })))).toEqual({
      supported: true,
      error: 'no API key',
