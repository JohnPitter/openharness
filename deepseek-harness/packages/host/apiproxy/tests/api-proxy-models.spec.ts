...
describe('Web session model selection', () => {
  ...
  it('omits a registered route the adapter does not advertise from the picker', async () => {
    const { ctx, sessionId } = await harness()
    ctx.llm.registerAdapter(['dormant'], new class extends CatalogAdapter {
      override advertiseModels(): Promise<boolean> {
        return Promise.resolve(false)
      }
    }('Dormant', [{ provider: 'dormant', id: 'hidden', name: 'Hidden' }]))
    const api = createApiProxy(ctx, { defaultModelSelection: () => ({ provider: 'deepseek-official', model: 'deepseek-chat' }), cwd: '/tmp' })
    const catalog = expectValue(await api.sessions.models(request({ sessionId })))
  ...
  it('blocks a session whose current route is registered but not advertised', async () => {
    const { ctx, sessionId } = await harness({
      provider: 'dormant',
      model: 'hidden',
    })
    ...
    expect(refused.result).toMatchObject({
      ok: false,
      error: { code: 'model-unavailable', details: { provider: 'dormant', model: 'hidden' } },
    })
    await ctx.fiber.dispose()
  })

  it('forwards adapter-disclosed context windows onto catalog models', async () => {
    const { ctx, sessionId } = await harness()
    ctx.llm.registerAdapter(['wide'], new class extends CatalogAdapter {
      override resolveModel(provider: string, model: string): Promise<LlmResolvedModelInfo> {
        return Promise.resolve({
          provider,
          id: model,
    ...
    }('Wide', [{ provider: 'wide', id: 'wide-1', name: 'Wide One' }]))
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'deepseek-official', model: 'deepseek-chat' }),
      cwd: '/tmp',
    })
    const catalog = expectValue(await api.sessions.models(request({ sessionId })))
    expect(catalog.groups.find(group => group.id === 'wide')?.models).toEqual([
      { id: 'wide-1', name: 'Wide One', contextWindow: 262_144 },
    ])
    await ctx.fiber.dispose()
  })
  ...
  it('replaces a stored effort the current model no longer advertises', async () => {
    const k3Reasoning: LlmModelReasoningInfo = {
      efforts: [
    ...
    ctx.llm.registerAdapter(['kimi-for-coding'], new CatalogAdapter(
      'Kimi for Code',
      [{ provider: 'kimi-for-coding', id: 'k3-256k', name: 'K3-256k' }],
      k3Reasoning,
    ))
    const api = createApiProxy(ctx, {
