 ...
 * host-scoped `llm.models`. Catalog membership stays advisory: an unlisted
 * session selection remains valid for provider dispatch, but is not injected
 * back into the selector after its owning catalog stops advertising it.
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /**
   * Drop a stored effort the current model no longer advertises (e.g. K3
   * after a DeepSeek/K2 `off`, or a capability change in a resumed log).
   * The selector remaps illegal efforts for display only; without this the
   * next `prepareCall` still sends the stale value and the turn fails.
   */
  async function healSelectionEffort(agent: Agent): Promise<void> {
    const current = selectionFor(agent).current
    if (current.reasoningEffort === undefined || !routeServed(current.provider)) return
    try {
      ...
      if (!(error instanceof LlmError) || error.code !== 'UNSUPPORTED_REASONING_EFFORT') return
      try {
        ...
        // Capability lookup failed; the turn reports the original error.
      }
    }
  }
  ...
  /**
   * Whether an adapter currently serves this provider. Catalog membership
   * cannot answer it: an adapter may serve a model its own catalog stopped
   * advertising, so a provider missing from the groups is not the same as
   * one nothing serves. A composition with no llm registry at all cannot
   * judge and says yes — the dispatch it would have refused fails on its
   * own terms.
   */
  ...
  /**
   * Whether the picker (and a new turn) should treat this route as enabled:
   * registered, and the adapter currently advertises it (credential or OAuth
   * present, or no credential required). A registered Codex/Claude route
   * without a key stays in Settings but must not stay selectable.
   */
  async function routeAdvertised(provider: string): Promise<boolean> {
    if (!routeServed(provider)) return false
    const llm = ctx.get('llm')
    return llm === undefined || llm.advertiseModels(provider)
  }

  /**
   * Resolve the addressed agent for a turn-starting method and refuse when no
   * adapter serves its current selection, or the adapter does not currently
   * advertise that route (no API key / OAuth). A provider nothing can talk to
   * cannot start a turn; refusing here names the model the session is pointed
   * at while the draft is still in the composer.
  ...
  async function turnAgentFor<T>(
    ...
    await healSelectionEffort(agent)
    const selection = selectionFor(agent).current
    if (!await routeAdvertised(selection.provider)) {
      return {
        refused: err(request, {
  ...
  return {
    sessions: {
      ...
      async models(request) {
        ...
        const found = await agentFor(sessionId)
        if ('error' in found) return err(request, found.error)
        await healSelectionEffort(found.agent)
    ...
    llm: {
      ...
      async accountUsage(request, signal) {
        const { provider } = request.payload
        try {
          const usage = await ctx.llm.accountUsage(provider, signal)
          if (usage === undefined) return ok(request, { supported: false })
          return ok(request, {
            supported: true,
            ...usage.plan === undefined || usage.plan.length === 0 ? {} : { plan: usage.plan },
          ...
          if (error instanceof LlmError && error.code === 'NO_ADAPTER') {
            return ok(request, { supported: false })
          }
          return ok(request, {
            supported: true,
