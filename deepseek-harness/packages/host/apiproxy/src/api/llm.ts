...
export interface LlmApi {
  ...
  /**
   * Account-level quota for one provider route, using the stored credentials
   * already configured for generation. `supported: false` when the adapter
   * has no quota surface (or the route is unregistered). Failures to read
   * the quota (missing key, AUTH, transport) stay on `error` rather than
   * failing the RPC — the chip still has session meters to show.
   */
  accountUsage(
    request: RpcRequest<{ provider: string }>,
    signal?: AbortSignal,
  ): Promise<RpcResponse<AccountUsageView>>
}
...
/** One account-level quota window on the llm.accountUsage wire. */
export interface AccountUsageWindowView {
  ...
  percent: number
  /** Unix seconds when this window resets, when disclosed. */
  resetsAt?: number
  /** Rate-limit window length in minutes, when disclosed. */
  windowMinutes?: number
}

/** Wire view of llm.accountUsage. */
export interface AccountUsageView {
  /** Whether this provider exposes an account-quota surface. */
  supported: boolean
  /** Human-readable failure when the quota could not be read. */
  error?: string
  /** Human-readable plan name when the adapter infers one. */
  plan?: string
