...
import type { ConfigurableProviderView, DiscoveredModelView, AccountUsageWindowView } from './llm.ts'
...
/** AccountUsageWindowView row of llm.accountUsage. */
export const accountUsageWindowViewSchema = z.object({
  id: z.string().min(1),
  used: z.number(),
...
}) satisfies z.ZodType<Wire<AccountUsageWindowView>>

/** llm.accountUsage request payload. */
export const llmAccountUsageRequestSchema = z.object({
  provider: z.string().min(1),
}) satisfies z.ZodType<Wire<RequestPayload<'llm.accountUsage'>>>

/** llm.accountUsage response value. */
export const llmAccountUsageValueSchema = z.object({
  supported: z.boolean(),
  error: z.string().min(1).optional(),
  plan: z.string().min(1).optional(),
  windows: z.array(accountUsageWindowViewSchema).optional(),
}) satisfies z.ZodType<Wire<ResponseValue<'llm.accountUsage'>>>
