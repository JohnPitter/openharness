...
| `foregroundWait` | 本实例是否允许在前台等待，默认 `allowed`。`never` 会省略 `run_in_background`、始终走后台路由，并忽略多余的 `false`；要求后台执行可用。 |
