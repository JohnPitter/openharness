...
`SubagentStartRequest.label` 是由会话支撑的一次性 child 所使用的可选简短持久化显示标签。面向模型的委派会提供其已有的 `description`；底层调用方无需凭空构造展示元数据。可继续启动始终携带自身的必填标签。`signal` 是必填项，也是一次性 `start` 的规范取消通道。发布前中止会使 `start()` 在回滚后拒绝；发布后中止会取消已返回 run 的剩余轮次工作，但不会隐藏其 id。请求还可以选择模型、要求结构化输出、限制委派深度、约束子 agent 工具或设置子 agent persona。当父会话是 Workflow 且规划器路由按请求计费时，`resolveChildAgentOptions` 拒绝继承该路由，除非已选择工人 chip 或请求里点名了提供方（`WorkflowWorkerRequiredError`）；按 token 计费的规划器在 chip 为空时仍继承。该继承规则由[路由计费 Agent Note](../../../.agents/notes/implemented/feature/2026-08-23-route-metering.zh.md) 拥有。对于可继续启动或后续操作，调用方信号只负责 inbox 接受前的查找、物化和准入；此后，Activation 由管理器独立拥有，因此调用方取消既不会取消已接受的轮次，也不会 dispose（资源释放）子 agent。
