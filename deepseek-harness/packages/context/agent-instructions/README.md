...
Root discovery climbs only when a marker probe confirms that the marker is absent. A permission or I/O failure stops discovery and surfaces the host or filesystem-provider error instead of selecting an ancestor project. The [root-marker failure policy](../../../.agents/notes/implemented/bug-fix/2026-09-08-stream-id-windows-hide-root-marker.md) records why discovery fails instead of substituting another root.
