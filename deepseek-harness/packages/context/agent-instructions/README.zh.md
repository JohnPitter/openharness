...
只有确认项目根标记不存在时，项目根发现才会继续上溯。权限或 I/O 失败会停止发现，并抛出宿主或文件系统提供方的原始错误，而不会选择祖先项目。[根标记失败策略](../../../.agents/notes/implemented/bug-fix/2026-09-08-stream-id-windows-hide-root-marker.zh.md)说明发现为何必须失败，而不能替换为其他根目录。
