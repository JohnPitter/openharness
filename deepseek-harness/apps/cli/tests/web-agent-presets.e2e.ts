...
describe('the shipped Web composition', () => {
  ...
  it('supplies both shipped presets, and only those, from the system root', async () => {
    ...
    expect(listed.map(preset => preset.id).sort()).toEqual(['code', 'cordis', 'minimal', 'standard', 'workflow'])
  ...
  it('composes the full agent from `standard`', async () => {
    ...
    try {
      ...
      expect(toolNames(ctx, handle.agent).filter(name => name !== 'glob' && name !== 'grep')).toEqual([
        ...
        'get_goal', 'interrupt_agent', 'job_kill', 'job_list', 'job_output', 'list_agents', 'milestone_write', 'ralph', 'read', 'read_image', 'send_message', 'skill',
  ...
  it('ships J-Space as the construction protocol for standard, code, and workflow', async () => {
    const skill = join(CONFIG_DIR, 'skills', 'j-space', 'SKILL.md')
    expect(await readFile(skill, 'utf8')).toMatch(/^---\r?\nname: j-space\b/u)

    for (const id of ['standard', 'code', 'workflow'] as const) {
      const handle = await ctx.agents.create({
        sessionId: SessionId(`preset-jspace-${id}-${randomUUID()}`),
      ...
      try {
        ...
        const loaded = await ctx.tools.execute({
          ...
          name: 'skill',
          arguments: { name: 'j-space' },
          signal: new AbortController().signal,
          agent: handle.agent,
        })
        expect(loaded.isError).toBe(false)
        expect(JSON.stringify(loaded.content)).toContain('The J-Space Premise')
      } finally {
        await handle.dispose()
      }
    }
...
describe('a switch survives the session', () => {
  it('records the choice so the log states what the agent runs', async () => {
    ...
    try {
      // The api-proxy's select does exactly this pair after a switch commits.
  ...
  it('rebuilds a switched session from the log, not the creation header', () => {
    ...
    // the log records the switch the user made.
    ...
    // history another one produced — the replay resolveSessionPreset prevents.
