...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('persists automatic compaction and its threshold across reload', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-settings-compaction'))
    await page.getByRole('button', { name: '设置', exact: true }).click()
    const dialog = page.getByRole('dialog', { name: '设置' })
    await dialog.waitFor({ timeout: 10_000 })
    await dialog.getByRole('button', { name: '75%' }).click()
    await page.getByRole('menuitem', { name: '50%' }).click()
    await dialog.getByRole('button', { name: '50%' }).waitFor({ timeout: 10_000 })
    await dialog.getByRole('button', { name: '开启' }).click()
    await page.getByRole('menuitem', { name: '停用' }).click()
    await dialog.getByRole('button', { name: '停用' }).waitFor({ timeout: 10_000 })
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/compaction-basic:[\s\S]*auto: false/)
    await expect.poll(async () => readFile(join(scaffold.harnessHome, 'settings.yaml'), 'utf8'), { timeout: 5_000 })
      .toMatch(/compaction-basic:[\s\S]*thresholdPercent: 50/)
    await page.keyboard.press('Escape')
    expect(tripwire.pageErrors).toEqual([])
  }, 90_000)
