...
An agent turn interleaves many tool-call and command nodes, and rendering each as its own card makes the conversation surface unreadable during long exploration or editing runs. A turn's opening context injections (system prompt, skill catalog, a recalled session) and a leading Think step have the same problem: each already renders as its own individually-collapsed row, so a turn can open with three or four separate rows before any visible reply. Condensing the run cannot lose information: the user still needs per-category counts, a live in-progress signal, and a way to reach the original cards in order, and anchors such as user text, assistant text, compaction, milestones, and turn tails must keep their own rendering and positions.

## Decision

Grouping is a pure view-layer fold. `groupActivityNodes()` in `packages/client/ui-conversation/src/client/chat/activity-groups.ts` scans the root chat-node list once and folds every run of two or more consecutive activity nodes into one `ActivityGroup` carrying the original nodes in order, per-category counts (`context`, `explored`, `edits`, `searches`, `commands`, `web`, `subagents`, `other`), and a running flag derived from the member states. Activity nodes are tool calls, commands, logged `context` injections (never running — the content is already resolved), and assistant steps whose blocks are tool-call and/or reasoning only; an assistant step carrying a visible text block is the turn's reply and anchors like a user message. A reasoning-only assistant step (a Think summary) is a transparent member: it joins the run and renders in its original position on expansion, but adds no category count, so interleaved reasoning never splits one turn's work into separate summary rows — including a Think that opens a turn right after its context injections, which now condense into the same row. A run whose members are all transparent (zero total count) renders its nodes individually instead of a header-less group. Any other anchor node — user or assistant text, compaction card, milestone, turn tail/retry/max-tokens — breaks the run and renders normally, and a singleton activity node (including a lone context injection) also renders normally; nothing is folded that cannot be restored exactly.
...
No subsystem reference owns the chat activity surface, so this note is the decision record; the chat UI surface is not documented under `docs/subsystems`. Focused component and unit coverage under `packages/client/ui-conversation` exercises run detection and anchor breaks in `groupActivityNodes`, singleton passthrough, per-category counting, the running flag, chevron expand/collapse ordering, per-session transient expansion state across remounts, the zh/en/pt/es summary copy, the reasoning-transparency regression (a Think step no longer splits one run into two summary rows, a transparent step keeps its expansion order without counting toward any category, a run of only transparent steps falls back to individual nodes instead of a header-less group, and a step mixing reasoning with an inline tool-call block counts under `other`), and the turn-opening regression: consecutive context injections condense into one `context`-counted row, a context run absorbs a following Think without counting it, and a lone context injection still renders individually.
