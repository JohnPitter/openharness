...
会话 UI 把 `milestone/write` 折成 `milestone` Chat Node（折叠 chip，可展开正文），左侧轨列出里程碑标题和用户消息航点，并通过 `[data-chat-anchor-key]` 跳转。该轨放在铺满流程的 overlay 里，以从顶部按 8px 节奏叠起的刻度迷你图对齐会话滚动口；栈高出粘滞栏时轨道滚动并跟随尾部。悬停或点击某个航点只打开该条目的视口固定预览，钳位后会话滚动口和编辑器无法裁切；Escape 或在路轨外按下指针即回到刻度（轨道仅在溢出时接收指针事件，短栈不会挡住正文侧沟）。会话栏不超过 840px 时，预览从左侧内边距覆盖 transcript，轨道保持 sticky。
