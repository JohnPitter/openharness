...
**Every permission picker gates `danger-full-access` behind the shared in-page `RiskConfirmation` dialog whose enabling action stays disabled until an explicit acknowledgement checkbox is checked; the preset renders under the locale product label; every dismissal path submits nothing.**
...
- The locale product label (`access.fullAccess` / `preset.fullAccess`, currently 完全访问 / Full access / Acesso total / Acceso total) overrides the kebab-to-title display transform in every picker; confirm copy interpolates that name; command and Settings writes keep the machine name on the wire.
