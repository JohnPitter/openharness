...
2. **输入框的发送/停止按钮反映草稿内容，而不只是运行状态**（`packages/client/ui-conversation/src/client/skeleton/InputBar.tsx`）：轮次运行期间，主按钮现在只有在输入框为空或被阻塞时才显示"停止"；有可发送草稿时显示"发送"，而不是强制先点一次停止。该发送的投递方式跟随 busy-Enter 偏好（[composer 繁忙发送、Think 摘要与空提示](../bug-fix/2026-09-08-composer-busy-send-think-empty.zh.md)）。直接移植；本分支的 `empty`／`blocked` 局部变量与上游修复前的形态完全一致。
