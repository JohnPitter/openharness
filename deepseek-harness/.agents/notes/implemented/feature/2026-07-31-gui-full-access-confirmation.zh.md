...
**每个权限选择器都把 `danger-full-access` 关进共享的页面内 `RiskConfirmation` 对话框：启用按钮在用户勾选明确的风险确认复选框前保持禁用；预设以 locale 产品标签展示；所有取消路径都不作任何提交。**
...
- locale 产品标签（`access.fullAccess` / `preset.fullAccess`，现为完全访问 / Full access / Acesso total / Acceso total）在每个选择器中覆盖 kebab 转 Title Case 的显示变换；确认文案内插该名称；命令与 Settings 写入在 wire 上保留机器名。
