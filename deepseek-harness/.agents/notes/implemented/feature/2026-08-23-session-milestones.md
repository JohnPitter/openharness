...
The conversation UI folds `milestone/write` into a `milestone` Chat Node (collapsed chip, expandable body) and a left rail of milestone titles plus user-message waypoints that jump through `[data-chat-anchor-key]`. The rail lives in a full-flow overlay and sticks to the conversation scrollport as a viewport-height tick minimap so a long session never grows a native list scrollbar. Hover or a waypoint click opens a floating preview for that item only; Escape or a pointer down outside the rail returns to ticks (tick buttons keep pointer events, so a collapsed rail never blocks the transcript gutter). On conversation columns 840px or narrower the preview overlays the transcript from the left padding and the track stays sticky.
