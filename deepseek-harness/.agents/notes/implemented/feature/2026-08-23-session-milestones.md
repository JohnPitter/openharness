...
The conversation UI folds `milestone/write` into a `milestone` Chat Node (collapsed chip, expandable body) and a left rail of milestone titles plus user-message waypoints that jump through `[data-chat-anchor-key]`. The rail lives in a full-flow overlay and sticks to the conversation scrollport as a tick minimap packed from the top on an 8px rhythm; when the stack exceeds the sticky column the track scrolls and follows its tail. Hover or a waypoint click opens a viewport-fixed preview for that item only, clamped so the conversation scroller and composer cannot clip it; Escape or a pointer down outside the rail returns to ticks (the track takes pointer events only while it overflows, so a short rail never blocks the transcript gutter). On conversation columns 840px or narrower the preview overlays the transcript from the left padding and the track stays sticky.
