...
The conversation UI folds `milestone/write` into a `milestone` Chat Node (collapsed chip, expandable body) and a left rail of milestone titles plus user-message waypoints that jump through `[data-chat-anchor-key]`. The rail lives in a full-flow overlay and sticks to the conversation scrollport so a long transcript does not scroll it off-screen.
