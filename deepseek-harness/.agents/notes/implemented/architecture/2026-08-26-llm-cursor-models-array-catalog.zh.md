...
- 只有当 `x-cursor-client-version` 仍被后端接受时，实时 `GetUsableModels` 才会把 Grok 以及该账户目录的其余模型填进选择器（[版本钉扎](../../bug-fix/2026-08-26-llm-cursor-client-version-rejected.zh.md)），并且一元响应按 Connect 帧与 `model_names` 解码（[列举解码](../../bug-fix/2026-08-27-llm-cursor-usable-models-connect-frames.zh.md)）。
