...
`dsh-llm-pi-ai` implements the wire path in [`src/discovery.ts`](../../../packages/llm/llm-pi-ai/src/discovery.ts). Catalog-route answers, draft credentials, and the size ceiling stay as decided here; the listing protocols, `models` map, Anthropic native list, profile `headers`, and name/capacity backfill are current in [model catalog search and discovery](../feature/2026-09-08-model-catalog-search-and-discovery.md).
...
What it costs: the wire gained a third secret-carrying payload, so the configuration plane's write-only surface is now three methods rather than two. Discovery coverage is still protocol-shaped — Azure, Codex, and Google remain `DISCOVERY_UNSUPPORTED`. And because nothing re-runs the question, a model list is still only as current as its last edit; that is the same trade the layer below made deliberately.
